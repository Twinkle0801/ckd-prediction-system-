...
def render_gauge(ckd_probability: float, prediction_label: str):
    ...
    FIX: previously this received a "confidence in the predicted class"
    value that flipped meaning depending on the outcome (probability for
    "ckd", 1-probability for "notckd"), while the gauge's background bands
    always assumed "higher number = more danger". That produced a visibly
    contradictory result -- e.g. a "Low Risk" verdict with the needle sitting
    in the red danger zone. Passing the raw CKD probability here keeps the
    number and the color bands always telling the same story.
    """
...
def render_confidence_badge(display_value: float, prediction: str):
    ...
    the PREDICTED class -- this one is fine to keep as "confidence", since
    it's labeled as such and not plotted next to risk-colored bands).
    Written as plain if/else blocks (no ternary-with-side-effects) so
    Streamlit never receives a DeltaGenerator object as an argument.
    """
...
if submitted:
    ...
    probability = result["probability"]  # probability of CKD (class 1) -- always this direction
    ...
    with result_col:
        ...
        if prediction == "ckd":
            ...
            confidence_value = probability          # confidence in "ckd" verdict
        else:
            st.success("## 🟢 Low Risk of CKD")
            confidence_value = 1 - probability       # confidence in "not ckd" verdict

        # FIX: gauge always plots the raw CKD probability, never the
        # class-dependent "confidence" value -- so the needle position and
        # the color bands (green=low risk, red=high risk) always agree with
        # the verdict shown above, regardless of which class won.
        render_gauge(probability, prediction)

        # Confidence badge intentionally still uses confidence_value -- it's
        # explicitly labeled "confidence", shown as a plain badge with no
        # risk-colored bands to contradict, so no fix needed here.
...
"""
    ...
    with explain_col:
        ...
        # Normalize progress bars against the largest |shap value| among the
        # top 5, instead of a hardcoded 1.0 ceiling -- keeps relative
        # differences visible even when all values are well below 1.0.
        ...
        for item in contributions:
            ...
            # FIX: prefer the value the clinician actually typed into the
            # form. Previously this always showed item["value"], which is
            # read off the SCALED/processed DataFrame SHAP was computed on
            # -- close to, but not exactly, what was entered (e.g. 13.27
            # instead of the typed 13.5), undermining trust in the display.
            # Engineered features (bun_creatinine_ratio, anemia_ckd_flag)
            # aren't in raw_input, so those still fall back to the
            # processed value, which is the only value that exists for them.
